...
const UpdateNewsModal: React.FC<props> = ({item, subCategories, show, setShow, submit}) => {
    ...
    const _onChange = (e) => {
        ...
        else if(e.target.name === 'category' && val === 0)
            err.category = true;
        ...
        if(e.target.name == 'category' && (val == '' ||  val != 'casino' && val != 'software' && val != 'slot'))
    ...
    const _onClick = async () => {
        ...
        setLoading(true);
        let sended: any = await submit({id, title, category, subCategory, description, image});
        console.log(sended);
        if(sended) {
            setShow(false);
            setError({
    ...
    const isValid = () => {
        ...
        if(category && (category != 'software' && category != 'slot' && category != 'casino'))
            err.subCategory = false;
